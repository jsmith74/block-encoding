...
\section{Introduction}
 Since the discovery of several high-profile quantum algorithms in the 1990's, there has been significant interest in developing a universal quantum computer that is both scalable and reliable. Among the wide variety of physical systems in which quantum behavior is observed, photon quanta have persisted as worthy candidates for carrying out information tasks. In particular, photons do not strongly interact with their environment and thus optical quantum states tend to resist decoherence better than other kinds of physical qudit states. On the other hand, photon-photon interaction also tends to be weak; linear entangling operations are limited to Hong-Ou-Mandel interference. Early proposals for an optical quantum computer circumvented this problem by encoding information into the degrees of freedom of a single photon, but this approach lacked scalability. In 2001, Knill Laflamme and Milburn discovered that universal entangling operations between qubits encoded into separate photons (i.e. the CNOT or CZ gate) could be achieved probabilistically using ancillla photons and partial measurements. Despite this, significant experimental effort continues to develop small single-photon devices for specialized applications, mostly in quantum networks~\cite{Starek,Barreiro,Graham,Lanyon}.
 
  In this paper, we explore a balance between single photon and multi-photon optical quantum computing. We propose encoding a few qubits into single-photon blocks so that within each block "entangling" operations between qubits are carried out through simple deterministic linear elements. Success of this model depends completely on reliable communication between blocks which is accomplished through photon-pair entangling operations. Here, we numerically test our ability to carry out these inter-block operations using only linear optical circuit elements  (i.e. beam splitters and phase shifters), ancilla states and probabilistic partial measurements.
