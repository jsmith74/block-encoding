...
\end{equation}
Each component $U_k$ can represent a simple beam splitter, phase shifter, or a general integrated circuit consisting of many low-level components.
...
\fbox{
	\begin{minipage}{23em}
		If the output states of our optical circuit, $\ket{\psi^\prime}$ are then projected onto some subspace of physical Hilbert space of the system, we need only build relevant rows of $A(U)$.
	\end{minipage}
}
\newline
\newline
...
\end{equation}
In the special case of $q=1$, our block encoding reduces to the standard dual-rail encoded qubit\cite{Dual Rail}. In the KLM~\cite{KLM,Uskov} scheme, the $\mbox{CNOT}_{first,last}$ gate can be applied to dual-rail qubits using probabilistic partial measurements with success probability $p=2/27$. In this paper, we use the protocol introduced in section~\ref{Section on Protocol} to numerically test our ability to construct the $\mbox{CNOT}_{first,last}$ gate using similar measurement-assisted transformations for $q>1$.
...
\end{center}
\end{table}
The focus of our work here was to test our ability to build these operators as probabilistic measurement-assisted transformations. By this, we mean to augment our optical computational state with an ancilla state, apply a linear optical transformation, and then do a partial projective measurement. The total action of this non-unitary process can be written formally as a Kraus operator called the \textit{PAULA} operator which acts on any optical quantum state $\ket{\psi_{c}}$ composed of $N_c$ photons in $M_c$ modes contained to some computational subspace;
\begin{equation}
...
\end{equation}
containing the same number of photons and modes in the ancilla state, $N_a,M_a$. $E$ therefore preserves photon and mode number. In accordance with \textit{Facts 3 and 4}, we do not construct the entire matrix $A(U)$ - only the columns mapped to by $L_a \ket{\psi_{in}}$ and the rows left after the projection $P$. We define $d_C$ as the dimension of the subspace all input states to our circuit are contained in, and $d_C^\prime$ as the full physical Hilbert space of our computational states;
\begin{equation}
	d_C^\prime = \frac{(N_C+M_C-1)!}{N_C! (M_C-1)!}
\end{equation}
$E$ can then be represented as a $d_C^\prime$ by $d_C$ rectangular matrix.

The gate fidelity of a \textit{PAULA} operator and unitary target operation $T$ is defined by
\begin{equation}
\label{Gate Fidelity}
F(E,T) = \frac{1}{d_C} \frac{\abs{\tr(E^\dagger T)}^2}{\tr(E^\dagger E)}.
\end{equation}
The probability of successfully applying $E$ after projective measurement $P$ is given by
\begin{equation}
\label{Success Probability} 
S(E) = \tr(E^\dagger E)
\end{equation}
For each $\mathcal{C}_1 \dots \mathcal{C}_4$ we want to simultaneously maximize the gate fidelity and success probability for each  $N_C=0,1,2$ over the same quantum optical circuit $U$ and ancilla state $\ket{\psi_a}$. We will demonstrate the details of this in the following section for $\mathcal{C}_1$. The main ideas can easily be extended $\mathcal{C}_2 \dots \mathcal{C}_4$, and we omit detailed discussion of those cases from this paper.
...
\small f(U,\psi_a) = F(E_1,T_1) \cdot F(E_2,T_2) +\epsilon S(E_1) \cdot S(E_2)
\end{equation}
where $\epsilon \in \{0,1\}$ is just a real numerical weight for the optimization.

If we achieve perfect fidelity, $F(E_1,T_1) \cdot F(E_2,T_2) = 1$, a difference in global phases on $T_1$ and $T_2$ will lead to an unwanted relative phase between Fock states of $N_C=1$ and $N_C=2$ when we apply the full operator $\mathcal{C}_1$. Fortunately, we can always correct for this. If we have a global phase difference of $e^{-i \phi}$ between $T_1$ and $T_2$, we simply apply a phase shift of $e^{i \phi}$ on every mode at the end of our circuit; we apply $A(U^\prime)$ such that
\begin{equation}
...
UP TO HERE FIGURE OUT 0 PHOTON TRANSFORMATION
\newline
